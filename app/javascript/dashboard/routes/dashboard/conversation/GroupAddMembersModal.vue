...
const selectedCountLabel = computed(() =>
  t('CONVERSATION.GROUP.ADD_MEMBERS_SELECTED', {
...
const addMembers = async () => {
  ...
  if (!participants.length) {
    useAlert(t('CONVERSATION.GROUP.ADD_MEMBERS_EMPTY'));
    return;
  }

  isSaving.value = true;
  try {
    const { data } = await conversationApi.addGroupContacts({
      conversationId: props.conversationId,
      participants,
    });

    if (data.failed?.length) {
      useAlert(data.error || data.failed.join(', '));
      return;
    }
    ...
    useAlert(
      error.response?.data?.error ||
        error.message ||
        t('CONVERSATION.GROUP.ADD_MEMBERS_ERROR')
    );
  } finally {
    ...
    <div class="flex max-h-[40rem] flex-col">
      <div class="border-b border-n-weak px-6 py-5">
        <h3 class="m-0 text-lg font-medium text-n-slate-12">
          {{ $t('CONVERSATION.GROUP.ADD_MEMBERS') }}
        </h3>
        <p class="m-0 text-sm text-n-slate-10">
          {{ $t('CONVERSATION.GROUP.ADD_MEMBERS_SUBTITLE') }}
        </p>
      </div>
      ...
      <div class="flex flex-1 flex-col gap-3 overflow-hidden px-6 py-4">
        ...
        <div class="flex justify-end gap-2">
          ...
          <Button
            color="blue"
            size="sm"
            :label="$t('CONVERSATION.GROUP.ADD_SELECTED_MEMBERS')"
