...
const allConversations = computed(() => store.getters['conversations/getAllConversations'] || []);
...
const syncColumns = () => {
  ...
  filteredConversations.value.forEach(conversation => {
    ...
    const matchedStage = activePipeline.value.stages.find(s => conversation.labels?.includes(s.label));
    if (matchedStage) {
...
watch([filteredConversations, activePipeline], () => {
  syncColumns();
...
const onCardDragChange = async (event, targetStage) => {
  if (event.added) {
    ...
    try {
      ...
      if (activePipeline.value.automations?.auto_resolve_on_won_lost && (targetStage.is_won || targetStage.is_lost)) {
...
const deleteActivePipeline = async () => {
  ...
  const remainingPipelines = fullConfig.value.pipelines.filter(p => p.id !== activePipeline.value.id);
...
const eligibleConversationsForInclusion = computed(() => {
  ...
  return allConversations.value.filter(c => {
    ...
    const hasPipelineLabel = c.labels?.some(lbl => allStagesLabels.includes(lbl));
...
<template>
  <div class="flex flex-col w-full h-full bg-slate-950 font-sans overflow-hidden">
    <!-- Header Top Section -->
    <header class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 px-6 py-4 border-b border-slate-900 bg-slate-950 shrink-0">
      ...
      <div class="flex items-center gap-3">
        ...
        <div v-if="fullConfig.pipelines.length > 0" class="flex items-center gap-1.5">
          <select
    ...
    >
      <div class="max-w-md p-6 border border-slate-800 bg-slate-900/50 rounded-2xl shadow-xl flex flex-col items-center">
        ...
        <p class="text-sm text-slate-400 mb-6 leading-relaxed">
          Nenhum funil Kanban foi configurado ainda para esta conta. Crie etapas personalizadas e gerencie seus leads de forma nativa e integrada.
        </p>
        <Button blue solid md class="flex items-center gap-2" @click="openAddPipeline">
    ...
    <main v-else class="flex-1 flex gap-5 p-6 overflow-x-auto select-none items-stretch">
      <div
      ...
      >
        <!-- Column Header -->
        <div class="p-4 border-b border-slate-900/60 flex items-center justify-between shrink-0 bg-slate-900/30">
          ...
          <span class="px-2 py-0.5 rounded-full bg-slate-900 text-[10px] font-bold text-slate-400 border border-slate-800">
            {{ columnsCardsMap[stage.id]?.length || 0 }}
          </span>
        </div>

        <!-- Draggable Cards Container -->
        <div class="flex-1 overflow-y-auto px-3.5 py-4 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
          <Draggable
        ...
        <div class="p-3 border-t border-slate-900/40 shrink-0 relative">
          <button
            ...
            @click="showAddCardPopoverId = showAddCardPopoverId === stage.id ? null : stage.id"
          >
          ...
          >
            <div class="px-3 py-1.5 border-b border-slate-800 text-[10px] uppercase font-bold text-slate-500">
            ...
            >
              ...
              <div class="flex flex-col min-w-0">
                <span class="text-xs font-semibold truncate">{{ conv.meta?.sender?.name || 'Cliente' }}</span>
